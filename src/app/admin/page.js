...
export default function AdminPage() {
  ...
  if (loading) return (
    <>
      ...
      <div className="page-content" style={{ minHeight:'100vh', display:'flex',
        alignItems:'center', justifyContent:'center' }}>
        <p style={{ fontFamily:'Cormorant Garamond,serif', fontSize:'20px',
          fontStyle:'italic', color:'var(--cream-muted)' }}>Cargando panel...</p>
      </div>
    </>
  );
      ...
      <nav className="luxury-nav">
        ...
        <div style={{ display:'flex', alignItems:'center', gap:'12px' }}>
          ...
          <a href="/" style={{ textDecoration:'none' }}>
            <button className="btn-ghost" style={{ padding:'7px 14px', fontSize:'10px' }}>Ver cartas</button>
          </a>
          <button onClick={logout} className="btn-ghost" style={{ padding:'7px 14px', fontSize:'10px' }}>Salir</button>
        </div>
      </nav>
        ...
        <div style={{ maxWidth:'800px', margin:'0 auto' }}>
          <div style={{ textAlign:'center', marginBottom:'50px' }} className="fade-in">
            ...
            <p style={{ fontFamily:'Cormorant Garamond,serif', fontSize:'16px',
              fontStyle:'italic', color:'var(--cream-muted)' }}>Solo tú puedes ver esta página</p>
          </div>
          ...
          <div style={{ display:'grid', gridTemplateColumns:'repeat(auto-fit,minmax(140px,1fr))',
            ...
            {[
              ...
              <div key={s.label} className="admin-card" style={{ textAlign:'center', padding:'24px 16px' }}>
                <div style={{ fontSize:'22px', color: s.green?'#4ade80':'var(--gold)', marginBottom:'10px' }}>{s.icon}</div>
                <div style={{ fontFamily:'Cinzel,serif', fontSize:'30px', fontWeight:600, color:'var(--gold-light)' }}>{s.value}</div>
                <div style={{ fontFamily:'Cinzel,serif', fontSize:'9px', letterSpacing:'2px',
                  textTransform:'uppercase', color:'var(--cream-muted)', marginTop:'6px' }}>{s.label}</div>
              </div>
            ))}
          </div>
          ...
          <div style={{ display:'flex', alignItems:'center', justifyContent:'flex-end', gap:'12px', marginBottom:'20px' }}>
            ...
            <button onClick={load} className="btn-ghost" style={{ padding:'7px 14px', fontSize:'10px' }}>Actualizar</button>
          </div>
          ...
          {users.length === 0 ? (
            <div className="admin-card" style={{ textAlign:'center', padding:'60px 20px' }}>
              ...
              <p style={{ fontFamily:'Cormorant Garamond,serif', fontSize:'20px',
                ...
                Nadie se ha registrado aún.<br />Comparte el link y espera...
              </p>
            </div>
          ) : (
              ...
              {[...online, ...offline].map(u => (
                <div key={u.id} className="admin-card" style={{
                      ...
                      <div>
                        <div style={{ display:'flex', alignItems:'center', gap:'10px', marginBottom:'3px' }}>
                          <span style={{ fontFamily:'Playfair Display,serif', fontSize:'17px',
                            fontStyle:'italic', color:'var(--cream)' }}>{u.name}</span>
                          {u.is_online ? (
                            ...
                            <span style={{ fontFamily:'Cinzel,serif', fontSize:'9px', letterSpacing:'1.5px',
                              textTransform:'uppercase', color:'var(--cream-muted)' }}>Desconectada</span>
                          )}
                        </div>
                        <div style={{ fontFamily:'EB Garamond,serif', fontSize:'14px',
                          color:'var(--cream-muted)', fontStyle:'italic' }}>{u.email}</div>
                      </div>
                    </div>
                    <div style={{ display:'flex', gap:'22px' }}>
                      <div style={{ textAlign:'center' }}>
                        <div style={{ fontFamily:'Cinzel,serif', fontSize:'18px', color:'var(--crimson-soft)' }}>♥ {u.total_likes}</div>
                        <div style={{ fontFamily:'Cinzel,serif', fontSize:'8px', letterSpacing:'1px',
                          textTransform:'uppercase', color:'var(--cream-muted)' }}>gustos</div>
                      </div>
                      <div style={{ textAlign:'center' }}>
                        <div style={{ fontFamily:'Cinzel,serif', fontSize:'18px', color:'var(--gold)' }}>✦ {u.total_comments}</div>
                        <div style={{ fontFamily:'Cinzel,serif', fontSize:'8px', letterSpacing:'1px',
                  ...
                  <div style={{ marginTop:'14px', paddingTop:'12px',
                    ...
                    {[
                      { label:'Cuenta creada', value: fmt(u.created_at) },
                      { label:'Última vez vista', value: u.last_seen ? fmt(u.last_seen) : 'Nunca', green: u.is_online },
                    ].map(d => (
                      <div key={d.label} style={{ display:'flex', alignItems:'baseline', gap:'6px' }}>
                        <span style={{ fontFamily:'Cinzel,serif', fontSize:'8px', letterSpacing:'1.5px',
                          textTransform:'uppercase', color:'var(--cream-muted)' }}>{d.label}:</span>
                        <span style={{ fontFamily:'EB Garamond,serif', fontSize:'14px',
                          fontStyle:'italic', color: d.green?'#4ade80':'var(--gold)' }}>{d.value}</span>
                      </div>
                    ))}
                  </div>
                </div>
